//===- LoopTensorizeCostModel.h ------------------------------------------*- C++
//-*-===//
//===----------------------------------------------------------------------===//

#ifndef LLVM_TRANSFORMS_TENSORIZE_LoopTensorizeCostModel_H
#define LLVM_TRANSFORMS_TENSORIZE_LoopTensorizeCostModel_H

#include "LoopTensorize.h"
#include "TPlanner.h"
...
struct FixedScalableVFPair {
  ...
  FixedScalableVFPair(unsigned rawVal)
      : FixedVF(ElementCount::getFixed(rawVal)),
        ScalableVF(ElementCount::getScalable(rawVal)) {}
...
public:
  LoopTensorizeCostModel(
      ScalarEpilogueLowering SEL, SmallVector<Loop *> Loops,
      MapVector<Loop *, PredicatedScalarEvolution *> Loop2PSE, LoopInfo *LI,
      LoopTensorizationLegality *Legal, const TargetTransformInfo &TTI,
      const TargetLibraryInfo *TLI, DemandedBits *DB, AssumptionCache *AC,
      OptimizationRemarkEmitter *ORE, const Function *F,
      const LoopTensorizeHints *Hints,
      MapVector<Loop *, InterleavedAccessInfo *> Loop2IAI)
      : ScalarEpilogueStatus(SEL), Loops(Loops), Loop2PSE(Loop2PSE), LI(LI),
        Legal(Legal), TTI(TTI), TLI(TLI), DB(DB), AC(AC), ORE(ORE),
        TheFunction(F), Hints(Hints), Loop2IAI(Loop2IAI) {}

  /// \return An upper bound for the vectorization factors (both fixed and
  /// scalable). If the factors are 0, vectorization and interleaving should be
  /// avoided up front.
  FixedScalableVFPair computeMaxVF(ElementCount UserTF, unsigned UserIC,
                                   Loop *CurL);
  ...
  bool isDivRemScalarWithPredication(InstructionCost ScalarCost,
                                     ...
                                     cl::boolOrDefault ForceSafeDivisor) const {
  ...
  bool isScalarWithPredication(Instruction *I, ElementCount VF, Loop *L) const;
  ...
  bool isPredicatedInst(Instruction *I, Loop *L) const;
  ...
  getDivRemSpeculationCost(Instruction *I, ElementCount VF, Loop *L) const;
  ...
  bool isAccessInterleaved(Instruction *Instr, Loop *L) const {
    auto *IAI = Loop2IAI.find(L)->second;
    return IAI->isInterleaved(Instr);
  }

  /// Get the interleaved access group that \p Instr belongs to.
  const InterleaveGroup<Instruction> *
  getInterleavedAccessGroup(Instruction *Instr, Loop *L) const {
    auto *IAI = Loop2IAI.find(L)->second;
    return IAI->getInterleaveGroup(Instr);
  }
  ...
  // /// A scalar epilogue must either be required for all VFs in \p Range or
  // for
  // /// none.
  ...
  //       "all VFs in range must agree on whether a scalar epilogue is
  //       required");
  //   return IsRequired;
  // }
  ...
  bool blockNeedsPredicationForAnyReason(BasicBlock *BB, Loop *L) const {
    return foldTailByMasking() || Legal->blockNeedsPredication(BB, L);
  }
...
private:
  ...
  FixedScalableVFPair computeFeasibleMaxVF(unsigned MaxTripCount,
                                           ElementCount UserTF,
                                           bool FoldTailByMasking, Loop *CurL);
  ...
  /// of elements.
  ElementCount getMaxLegalScalableVF(unsigned MaxSafeElements, Loop *CurL);
  ...
  DenseSet<Instruction *> InLoopReductionOperations;
  ...
  bool needsExtract(Value *V, ElementCount VF) const {
    ...
    if (VF.isScalar() || !I || !Loops.front()->contains(I) ||
        Loops.front()->isLoopInvariant(I))
      return false;
...
#endif // LLVM_TRANSFORMS_TENSORIZE_LoopTensorizeCostModel_H
